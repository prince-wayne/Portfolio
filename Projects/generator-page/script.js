...
// global consts used across functions 
const hList = document.querySelectorAll("h3"); // used for location / placement
let QA = ""; // initalizes str obj to later be set to answear from SR to be used in RA

function clearAll() {
	const paragraphs = document.querySelectorAll("p");
	const imgs = document.querySelectorAll("img");
	if (paragraphs.length > 2) {
		paragraphs.item(0).remove();
		paragraphs.item(1).remove();
...
function showMeme() {
	...
	img1.src = getRandomData("memes");
	hList.item(0).after(img1);	
}
...
function showJoke() {
	...
	clearAll();	
	const Joke1 = document.createElement("p");
	Joke1.textContent = getRandomData("jokes");
	hList.item(1).after(Joke1);	
}
...
function showQuote() {
	...
	clearAll();
	const obj = getRandomData("quotes");
	const str = obj.quote + " Author:  " + obj.author;
	const quote1 = document.createElement("p");
	quote1.textContent = str; // check this
	hList.item(2).after(quote1);
}
...
function showRiddle() {
	...
	clearAll();
	const obj = getRandomData("riddles");
	const Q = document.createElement("p");
	QA = obj.answer;
	Q.id = "Question";
	Q.textContent = obj.question;
	hList.item(3).after(Q);
}
...
function revealAnswers() {
	if (document.getElementById("Question") !== null && document.getElementById("Answer") == null) {
		...
		question.after(A);	
	} else if (document.getElementById("Question") !== null && document.getElementById("Answer") !== null) {
		window.alert("The Answer is displayed under the Question");
	}
	if (document.getElementById("Question") == null) {
		window.alert("Ask for a question first");
	}
